...
func (l *SearchPostLogic) SearchPost(in *pb.SearchReq) (*pb.SearchPostResp, error) {
	page, pageSize := normalizePage(in.Page, in.PageSize)
	if in.Keyword == "" {
		return &pb.SearchPostResp{
			Code: errutil.ErrInvalidParams,
			Msg:  errutil.GetCodeMessage(errutil.ErrInvalidParams),
		}, nil
	}

	cacheKey := redis.SearchResultKey + "post:" + getCacheKey(in.Keyword, page, pageSize)
	var cachedResp pb.SearchPostResp
	...
	if err != nil {
		return internalSearchPostResp(), nil
	}
	...
	list := make([]*pb.SearchPostItem, 0, len(esResp.Hits.Hits))
	for _, hit := range esResp.Hits.Hits {
		...
		list = append(list, &pb.SearchPostItem{
			Id:              doc.Id,
			UserId:          doc.UserId,
			Title:           doc.Title,
			Cover:           doc.Cover,
			...
			Visibility:      int32(doc.Visibility),
			IsTop:           doc.IsTop,
...
func internalSearchPostResp() *pb.SearchPostResp {
	return &pb.SearchPostResp{Code: errutil.ErrInternalError, Msg: errutil.GetCodeMessage(errutil.ErrInternalError)}
}
