...
func (l *GetFollowerListLogic) GetFollowerList(req *types.GetFollowerListReq) (resp *types.FollowListResp, err error) {
	if err := req.Validate(); err != nil {
		logx.Errorf("获取粉丝列表参数校验失败: %v", err)
	...
	rpcResp, err := l.svcCtx.RelationRpc.GetFollowerList(l.ctx, &pb.GetFollowerListReq{
		UserId:   userId,
		Page:     req.Page,
		PageSize: req.PageSize,
	})
	if err != nil {
		logx.Errorf("调用 RPC GetFollowerList 失败: %v", err)
...
func (l *GetFollowerListLogic) getUserIdFromContext() (uint64, error) {
