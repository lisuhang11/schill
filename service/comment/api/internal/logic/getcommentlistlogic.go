...
func (l *GetCommentListLogic) GetCommentList(req *types.GetCommentListReq) (resp *types.CommentListResp, err error) {
	rpcResp, err := l.svcCtx.CommentRpc.GetCommentList(l.ctx, &commentcenter.GetCommentListReq{
		PostId:   req.PostId,
		Cursor:   req.Cursor,
	...
	if err != nil {
		logx.Errorf("调用 RPC GetCommentList 失败: %v", err)
...
func (l *GetCommentListLogic) getUserIdFromContext() (uint64, error) {
